<?php $SUBTITLE="世界介绍" ?>
<?php include('headers.inc.html.php'); ?>
<?php $SUBINDEX=1 ?>
<?php include('nav.inc.html.php'); ?>
		<main class="bs-docs-masthead" id="content" role="main">
		<div class="container">
			<p class="lead">
			</p>
		</div>

		<div class="container">
			<div id="carousel-example-generic" class="carousel slide" data-ride="carousel">
				<ol class="carousel-indicators">
					<li class="" data-target="#carousel-example-generic" data-slide-to="0"></li>
					<li class="" data-target="#carousel-example-generic" data-slide-to="1"></li>
					<li class="" data-target="#carousel-example-generic" data-slide-to="2"></li>
					<li class="active" data-target="#carousel-example-generic" data-slide-to="3"></li>
				</ol>
				<div class="carousel-inner" role="listbox">
					<div class="item">
						<img src="img/game/1.png" alt="First slide">
					</div>
					<div class="item">
						<img src="img/game/2.png" alt="Second slide">
					</div>
					<div class="item">
						<img src="img/game/3.png" alt="Third slide">
					</div>
					<div class="item active">
						<img src="img/game/4.png" alt="Forth slide">
					</div>
				</div>
				<a class="left carousel-control" href="#carousel-example-generic" role="button" data-slide="prev">
					<span class="glyphicon glyphicon-chevron-left"></span>
					<span class="sr-only">Previous</span>
				</a>
				<a class="right carousel-control" href="#carousel-example-generic" role="button" data-slide="next">
					<span class="glyphicon glyphicon-chevron-right"></span>
					<span class="sr-only">Next</span>
				</a>
			</div>
		</div>
		</main>

		<div class="bs-docs-featurette">
			<div class="container">
				<div class="row">
					<div class="col-sm-4">
						<img src="img/zjy.jpg" alt="生存" class="img-responsive">
						<h3>生存</h3>
						<p>有什么什么设施</p>
					</div>
					<div class="col-sm-4">
						<img src="img/zjy.jpg" alt="游戏场" class="img-responsive">
						<h3>游戏场</h3>
						<p>MobArena PvPArena Spleef</p>
					</div>
					<div class="col-sm-4">
						<img src="img/zjy.jpg" alt="创作" class="img-responsive">
						<h3>创造</h3>
						<p>张嘉益是个建筑触手</p>
					</div>
				</div>
			</div>
		</div>
<?php include('foot.inc.html.php'); ?>
